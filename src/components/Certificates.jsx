  ...
  return (
    <section id="sertifikat" className="certificates">
      <div className="container">
        <h2 className="section-title">{t('cert.title')} <span className="text-gradient">{t('cert.title_highlight')}</span></h2>
        <p className="section-subtitle">{t('cert.subtitle')}</p>
        
        <div className="cert-grid">
          {certificatesData.map((cert, index) => (
            <div 
              key={cert.id} 
              className={`cert-card glass reveal ${cert.image ? 'clickable' : ''}`} 
